...
export const PinContainer = ({
	...
	return (
		<Link
			href={href}
			// Next.js’s Link emits exactly one <a> under the hood—no nesting anymore
			className={cn(
...
export const PinPerspective = ({
	...
	return (
		<motion.div
			className="pointer-events-none w-full h-80 flex items-center justify-center
                opacity-0 group-hover/pin:opacity-100 z-[60] transition duration-500"
		>
			<div className="w-full h-full -mt-7 flex-none inset-0">
				<div className="absolute top-0 inset-x-0 flex justify-center">
					{/* 
            Replace nested <a> with a <button> so we never have
            <a> → <a>. window.open only runs on click, so SSR markup
          ...
          */}
					...
					>
						...
						{/* fixed group-hover to target “pin” group, not “btn” */}
						<span
				...
				<>
					...
					/>
					<motion.div
						className="absolute right-1/2 translate-x-[1.5px]
                                bottom-1/2 bg-cyan-600 translate-y-[14px]
                                w-[4px] h-[4px] rounded-full z-40 blur-[3px]"
					/>
					<motion.div
						className="absolute right-1/2 translate-x-[0.5px]
